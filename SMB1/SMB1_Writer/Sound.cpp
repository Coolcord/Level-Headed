...
bool Sound::Randomize_Sounds() {
    ...
    case 2:     success = this->One_Up_3(); break;
    case 3:     success = this->One_Up_Random(); break;
    }
...
bool Sound::One_Up_3() { return this->Write_Bytes_To_Offset(0x74E4, this->Randomize_Notes(QByteArray::fromHex(QString("3430343A4040").toLatin1()))); }
...
bool Sound::Bowser_Drop_Random() {
    int highByte = Random::Get_Instance().Get_Num(0x7, 0xE);
    int lowByte = Random::Get_Instance().Get_Num(0x2, 0x6);
    if (!this->Write_Bytes_To_Offset(0x75DE, QByteArray(1, static_cast<char>((highByte*0x10)+lowByte)))) return false;
...
bool Sound::Kick_Random() {
    if (!this->Write_Bytes_To_Offset(0x748E, QByteArray(1, static_cast<char>(Random::Get_Instance().Get_Num(0x09, 0x0F))))) return false;
