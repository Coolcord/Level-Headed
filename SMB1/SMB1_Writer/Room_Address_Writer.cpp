...
Room_Address_Writer::Room_Address_Writer(QFile *file, Level_Offset *levelOffset) {
    ...
    this->lowObjectBuffer = new QByteArray(22, ' ');
    this->highObjectBuffer = new QByteArray(22, ' ');
    this->enemiesHeaderBuffer = new QByteArray(4, ' ');
    this->lowEnemyBuffer = new QByteArray(22, ' ');
    this->highEnemyBuffer = new QByteArray(22, ' ');
}
...
bool Room_Address_Writer::Read_Room_Address_Tables() {
    ...
    if (!this->Read_Into_Buffer(this->OBJECT_ADDRESS_LOW_BYTE, 22, this->lowObjectBuffer)) return false;

    //Read the Objects Address High Byte Table
    if (!this->Read_Into_Buffer(this->OBJECT_ADDRESS_HIGH_BYTE, 22, this->highObjectBuffer)) return false;
    ...
    if (!this->Read_Into_Buffer(this->ENEMY_ADDRESS_LOW_BYTE, 22, this->lowEnemyBuffer)) return false;

    //Read the Enemies Address High Byte Table
    if (!this->Read_Into_Buffer(this->ENEMY_ADDRESS_HIGH_BYTE, 22, this->highEnemyBuffer)) return false;
