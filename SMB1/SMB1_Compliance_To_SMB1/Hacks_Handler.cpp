...
bool Hacks_Handler::Write_Hacks() {
    ...
    if (fullSupport) {
        ...
        if (this->pluginSettings->randomBrickBreakAnimation && !this->writerPlugin->Hacks_Set_Brick_Break_Animation_Bounce_Height(Random::Get_Instance().Get_Num(0, 7), Random::Get_Instance().Get_Num(0, 7))) return false;
