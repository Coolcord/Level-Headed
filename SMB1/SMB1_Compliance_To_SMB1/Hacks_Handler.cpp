...
bool Hacks_Handler::Handle_Graphics() {
    ...
    if (palette == 0) palette = Random::Get_Instance().Get_Num(2, 10); //Random All
    else if (palette == 1) palette = Random::Get_Instance().Get_Num(3, 10); //Random (No Randomly Generated)
    if (palette > 3) {
...
bool Hacks_Handler::Handle_Lives() {
    ...
    if (this->pluginSettings->infiniteLives) {
        ...
        return this->writerPlugin->Hacks_Permadeath();
    } else {
        if (this->pluginSettings->alwaysRestartFromCurrentWorldAfterGameOver) this->writerPlugin->Hacks_Always_Force_Continue_Code();
        return this->writerPlugin->Hacks_Set_Starting_Lives(this->pluginSettings->numLives);
    }
}
